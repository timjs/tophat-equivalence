...
After evaluation is done, a task is ready to be \emph{fixated}.
Fixation is a big-step semantics that is responsible for reducing tasks until they are ready to accept input.
We write $t,\sigma,\delta \fixate t',\sigma'$ to denote the fixation of task $t$
in state $\sigma$ given the dirty set $\delta$, results into task $t'$ in state $\sigma'$.
The set $\delta'$ contains all heap locations whose value has been changed while fixation took place.
...
Fixation makes use of \emph{normalisation}, which is a helper semantics.

Input events are handled by the \emph{interaction} semantics.
For the interaction semantics, we write $t,\sigma \interact{\iota} t',\sigma'$ to denote that
task $t$ in state $\sigma$ transitions to task $t'$ in state $\sigma'$ after the user input $\iota$.
The interaction semantics makes use of a helper \emph{handling} semantics,
...
For further details we refer to \inlinecite[Steenvoorden22].
